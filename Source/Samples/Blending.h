...
public:
    ...
    VulkTextureView tileTextureView, fenceTextureView;
    VkSampler textureSampler;

    VulkMesh tileMesh;
    std::unique_ptr<MeshRenderBuffers> tileMeshRenderBuffers;
    VkDescriptorSetLayout tileDescriptorSetLayout;
    VkDescriptorPool tileDescriptorPool;
    VkPipelineLayout tilePipelineLayout;
    VkPipeline tileGraphicsPipeline;
    VkDescriptorSet tileDescriptorSets[MAX_FRAMES_IN_FLIGHT];
...
public:
    BlendingWorld(Vulk &vk) : vk(vk),
                              ...
                              fenceTextureView(vk, "Assets/Textures/Fence003_4K-JPG/Fence003_4K_Opacity.jpg")
    {
        textureSampler = vk.createTextureSampler();
        camera.lookAt(glm::vec3(0.0f, 0.f, 2.f), glm::vec3(0.f, 0.f, 0.f));

        makeQuad(1.f, 1.f, 1, tileMesh);

        tileMeshRenderBuffers = std::make_unique<MeshRenderBuffers>(vk, tileMesh.vertices, tileMesh.indices);

        tileDescriptorSetLayout = VulkDescriptorSetLayoutBuilder(vk)
        ...
        tileDescriptorPool = VulkDescriptorPoolBuilder(vk)
        ...
        VulkPipelineBuilder(vk)
            ...
            .build(tileDescriptorSetLayout, &tilePipelineLayout, &tileGraphicsPipeline);

        for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            tileDescriptorSets[i] = vk.createDescriptorSet(tileDescriptorSetLayout, tileDescriptorPool);
            VulkDescriptorSetUpdater(tileDescriptorSets[i])
    ...
    void updateXformsUBO(XformsUBO &ubo, VkViewport const &viewport) {
        ...
        ubo.proj = glm::perspective(glm::radians(45.0f), viewport.width / (float)viewport.height, 1.f, 4000.0f);
        ubo.proj[1][1] *= -1;
    }
    ...
    void render(VkCommandBuffer commandBuffer, uint32_t currentFrame, VkViewport const &viewport, VkRect2D const &scissor) {
        ...
        *frameUBOs.eyePos[currentFrame].mappedUBO = camera.eye;
        vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, tileGraphicsPipeline);
        vkCmdSetViewport(commandBuffer, 0, 1, &viewport);
        vkCmdSetScissor(commandBuffer, 0, 1, &scissor);

        VkDeviceSize offsets[] = {0};
        vkCmdBindVertexBuffers(commandBuffer, 0, 1, &tileMeshRenderBuffers->vertexBuffer, offsets);
        vkCmdBindIndexBuffer(commandBuffer, tileMeshRenderBuffers->indexBuffer, 0, VK_INDEX_TYPE_UINT32);
        vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, tilePipelineLayout, 0, 1, &tileDescriptorSets[currentFrame], 0, nullptr);
        vkCmdDrawIndexed(commandBuffer, (uint32_t)tileMesh.indices.size(), 1, 0, 0, 0);
    }

    ~BlendingWorld()
    {
        vkDestroySampler(vk.device, textureSampler, nullptr);
        vkDestroyDescriptorSetLayout(vk.device, tileDescriptorSetLayout, nullptr);
        vkDestroyDescriptorPool(vk.device, tileDescriptorPool, nullptr);
        vkDestroyPipelineLayout(vk.device, tilePipelineLayout, nullptr);
        vkDestroyPipeline(vk.device, tileGraphicsPipeline, nullptr);
    }
};
